...
gmirror <- function(top, bottom, tline, bline, chroms = c(1:22, "X", "Y"),log10=TRUE, 
                    ...
                    file="gmirror", hgt=7, hgtratio=0.5, wi=12, res=300, log_axis = FALSE){
  ...
  if("Shape" %in% topn){
    ...
    p1 <- p1 + geom_scattermore(data=d_order[d_order$Location=="Top",], aes(x=pos_index, y=pval, color=factor(Color)), alpha=opacity)
  }
  ...
  if("Shape" %in% bottomn){
    ...
    p2 <- p2 + geom_scattermore(data=d_order[d_order$Location=="Bottom",], aes(x=pos_index, y=pval, color=factor(Color)), alpha=opacity)
  }
  ...
  if(!missing(highlight_snp)){
    if("Shape" %in% topn){
      ...
      p1 <- p1 + geom_scattermore(data=d_order[d_order$SNP %in% highlight_snp & d_order$Location=="Top", ], aes(x=pos_index, y=pval), colour='green')
    }
    if("Shape" %in% bottomn){
      ...
      p2 <- p2 + geom_scattermore(data=d_order[d_order$SNP %in% highlight_snp & d_order$Location=="Bottom", ], aes(x=pos_index, y=pval), colour=highlighter)
    }
  }
  ...
  if(chrblocks==TRUE){
    ...
    if(log_axis==TRUE){
    p1 <- p1+theme(axis.text.x = element_text(vjust=1),axis.ticks.x = element_blank())+ scale_y_continuous(limits=c(yaxismin1, yaxismax1),expand=expansion(mult=c(0,0.1)),trans=scales::pseudo_log_trans(base = 10))

    p2 <- p2+scale_y_continuous(limits=c(yaxismax2,yaxismin2),expand=expansion(mult=c(0.1,0)),trans=pseudo_log_trans(base=10) %of% reverse_trans()) + theme(axis.text.x = element_blank(),axis.ticks.x = element_blank())} else{
      ...
      p2 <- p2+scale_y_continuous(limits=c(yaxismax2,yaxismin2),expand=expansion(mult=c(0.1,0)),trans=reverse_trans()) + theme(axis.text.x = element_blank(),axis.ticks.x = element_blank())
